...
if not os.path.isfile(f'/data2/xiongyuanpeng/gene_expression/data/{cell_line}/processed/baseline_data/annotation.csv'):
    ...
    rna_seq = rna_seq.rename(columns={'gene_id':'id','expected_count':'count'})
    def fun(x):
        if '.' in x:
            x = x.split('.')[0]
        return x
    ...
    result.to_csv(f'/data2/xiongyuanpeng/gene_expression/data/{cell_line}/processed/baseline_data/annotation.csv',index=False)
else:
    result = pd.read_csv(f'/data2/xiongyuanpeng/gene_expression/data/{cell_line}/processed/baseline_data/annotation.csv')
# 分染色体选择特征数据
resolution = 100
...
for i in range(1,20):
    ...
    fp1 = [float(v.strip().split()[1]) for v in open(data_dir+f'/{cell_line}/processed/H3K27ac_chr{i}.bedgraph')]
    fp2 = [float(v.strip().split()[1]) for v in open(data_dir+f'/{cell_line}/processed/H3K27me3_chr{i}.bedgraph')]
    fp3 = [float(v.strip().split()[1]) for v in open(data_dir+f'/{cell_line}/processed/H3K4me1_chr{i}.bedgraph')]
    fp4 = [float(v.strip().split()[1]) for v in open(data_dir+f'/{cell_line}/processed/H3K4me3_chr{i}.bedgraph')]
    fp5 = [float(v.strip().split()[1]) for v in open(data_dir+f'/{cell_line}/processed/H3K36me3_chr{i}.bedgraph')]
    for idx, row in tqdm(chr_data.iterrows()):
        try:
            ...
            ids = row['id']
            pkl.dump(data, open(data_dir+f'{cell_line}/processed/baseline_data/{chrs}_{ids}.bin','wb'))
        except:
            print(tts, tss)
            continue